...
export class ArrayAssertion extends AAssertion {
  public item: ValueDefinition;
  ...
  public tryParse(target: Target, options: Readonly<ParsingOptions>): any {
    ...
    if (value instanceof Array) {
      ...
      for (let index = 0; index !== length; ++index) {
        result.push(this.item.parse(new ArrayElement(value, index, target), options));
      }
      return value;
    } else {
      return undefined;
  ...
  public trySerialize(target: Target, options: Readonly<SerializationOptions>): any {
    ...
    if (value instanceof Array) {
      ...
      const length = value.length;
      for (let index = 0; index !== length; ++index) {
        result.push(this.item.serialize(new ArrayElement(value, index, target), options));
      }
      return value;
    } else {
      return undefined;
